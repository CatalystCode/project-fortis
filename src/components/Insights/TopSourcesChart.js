...
export const TopSourcesChart = React.createClass({
  ...
  initializeChart()
  {
      const {dataSource, siteKey} = this.props;
      const {datetimeSelection, timespanType} = this.props;
 ...
 refreshChart(){
    ...
    if(topSources){
        let dataProvider = topSources.map((source, index) => {
            let dataSourceSchema = Actions.DataSourceLookup(source.Source);
            let icon = <i style={styles.sourceLogo} className={`${dataSourceSchema.icon} fa-1x`}></i>;
 ...
 componentWillReceiveProps(nextProps){
     ...
     }else if(this.props.mainEdge !== nextProps.mainEdge || this.props.originalSource !== nextProps.originalSource ||
              (this.status === 'loading' && topSources.length > 0) || (this.props.datetimeSelection !== nextProps.datetimeSelection) || 
