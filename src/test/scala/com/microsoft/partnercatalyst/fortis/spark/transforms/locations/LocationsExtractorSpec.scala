...
class TestFeatureServiceClient(given: Seq[FeatureServiceFeature]) extends FeatureServiceClient("test-host") {
  override def bbox(geofence: Geofence): Iterable[FeatureServiceFeature] = given
}
