...
class LocationsExtractorSpec extends FlatSpec {
  "The locations extractor" should "extract single location from text" in {
    ...
    val locations = extractor.analyze(sentence).toSet
    assert(locations == Set(Location(geometry = Some(extractor.idNyc))))
  }
  ...
  it should "extract multiple locations from text" in {
    ...
    val extractor = new TestLocationsExtractor().buildLookup()
    val locations = extractor.analyze(sentence).toSet
    assert(locations == Set(Location(geometry = Some(extractor.idManhattan)),
      Location(geometry = Some(extractor.idNyc))))
  }
  ...
  it should "handle point queries" in {
    ...
    val points = extractor.fetch(12, 34)

    assert(points == Seq(Location(geometry = Some("id1"))))
  }
}
